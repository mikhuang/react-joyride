...
var Tooltip = React.createClass({
    ...
    componentDidMount: function() {
        this._checkClassOverlay(null, this.props);
    },

    componentDidUpdate: function(prevProps) {
        if (!this._isUnmounting) {
            this._checkClassOverlay(prevProps, this.props);
        }
    },

    componentWillUnmount: function() {
        this._isUnmounting = true;
        this._checkClassOverlay(this.props, null);
    },

    _checkClassOverlay: function(prevProps, props) {
        // check to see if we need to remove an existing overlay
        if (prevProps && prevProps.step.useClassOverlay) {
            document.querySelector(prevProps.step.selector) && document.querySelector(prevProps.step.selector).classList.remove('joyride-hole-static');
        }
        // check if we need to put in a new one
        if (props && props.step.useClassOverlay) {
            document.querySelector(props.step.selector) && document.querySelector(props.step.selector).classList.add('joyride-hole-static');
        }
    },
    ...
    handleOnClick: function(e) {
        ...
        // prevent close when noCloseButton is present
        if (type !== 'close' || !this.props.step.noCloseButton) {
    ...
    render: function () {
        ...
        opts.tooltipElement = React.createElement('div', {
            ...
            (step.noCloseButton ? false : React.createElement('a', {
                href: '#',
            ...
            React.createElement('div', {
                ...
                (props.buttons.secondary ?
                    ...
                    : false),
                (step.noNextButton ? false : React.createElement('a', {
                    href: '#',
                    className: 'primary',
        ...
        if (props.showOverlay && !step.useClassOverlay) {
